...
function StayBookingComp({cleanSelection, hidecategories, searchStays}: IStayBooking) {
  ...
  const userType: 'user' | 'host' = useSelector((state: iStoreState) => state?.user?.userMode);
  ...
  const listProperties = () => {
    navigate(`/${routeConstants.profile}?tab=properties`);
  }
  ...
  return (
    <div className='stay-booking-comp'>
      <div className={'stay-type-selector' + (hidecategories ? '' : ' floated-type')}>
        {userType === 'host' && <button className='purple-button purple-shadow' onClick={listProperties}>List Properties</button>}
      </div>
      <div className='row'>
