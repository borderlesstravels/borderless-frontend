...
function BookingComp(props: iProps) {
  const [mode, setMode] = useState<"FLIGHTS" | "STAYS" | "RIDES">(
    props.mode || "FLIGHTS"
  );
    ...
    <div className="booking-comp">
      <div className="booking-tab-space">
        {!props.hidecategories && (
          <div className="tabs">
            <div
              className={"tab" + (mode === "FLIGHTS" ? " active-tab" : "")}
            ...
            <div
              className={"tab" + (mode === "STAYS" ? " active-tab" : "")}
              onClick={() => switchMode("STAYS")}
            >
              <FontAwesomeIcon icon={"bed"} /> <span>Stays</span>
            </div>
            <div className="separator"></div>
            <div
              className={"tab" + (mode === "RIDES" ? " active-tab" : "")}
              onClick={() => switchMode("RIDES")}
            >
        ...
        <div className="main-content">
          ...
          {mode === "STAYS" && (
            <StaysBookingComp
              hidecategories={props.hidecategories}
              searchStays={props.searchStays}
              cleanSelection={props.cleanSelection}
            />
          )}
          {mode === "RIDES" && (
            <RideBookingComp cleanSelection={props.cleanSelection} />
          )}
        </div>
      </div>
    </div>
