...
function FlightBookingComp({
  ...
  const [combinedFlightData, setCombinedFlightData] =
    useState<ICombinedFlightSearchData>(
      cleanSelection
        ? generateNewCombinedFlightData()
        : storedCombinedFlightData
    );
  ...
  const [flightType, setFlightType] = useState<"return" | "one-way">(
    combinedFlightData.flightType
  );
  const [location, setLocation] = useState<ILocationData | undefined>(
    combinedFlightData.location
  );
  const [date, setDate] = useState<IDateData | undefined>(
    combinedFlightData.date
  );
  const [flightClass, setFlightClass] = useState<IFlightClassData | undefined>(
    combinedFlightData.flightClass
  );
  ...
  const compileCombinedData = () => {
    const tempCombination = {
      ...
      date:
        (flightType === "return" && date?.startDate && date?.endDate) ||
        (flightType === "one-way" && date?.startDate)
          ? date
          : undefined,
    ...
    if (
      tempCombination.location &&
      tempCombination.date &&
      (tempCombination.flightClass?.adultCount || 0) >=
        (tempCombination.flightClass?.infantCount || 0)
    ) {
      setCanProceed(true);
  ...
  return (
    <div className="flight-booking-comp">
      <div
        className={
          "flight-type-selector" + (hidecategories ? "" : " floated-type")
        }
      >
      ...
      <div className="row">
        ...
        <div
          className={
            flightType === "return" ? "col-lg-5 col-sm-8" : "col-lg-4 col-sm-8"
          }
        >
          <DateSelectionComp
            date={date}
            setDate={updateDate}
            multiple={flightType === "return"}
          />
        </div>
