...
function FlightClassSelectionComp(props: iFlightClassProps) {
  ...
  const [adultCount, setAdultCount] = useState(
    props.flightClass?.adultCount || 1
  );
  const [childrenCount, setChildrenCount] = useState(
    props.flightClass?.childrenCount || 0
  );
  const [infantCount, setInfantCount] = useState(
    props.flightClass?.infantCount || 0
  );
  const [allPassengerCount, setAllPassengerCount] = useState(
    props.flightClass?.allPassengerCount || 0
  );
  const [cabinClass, setCabinClass] = useState<
    "Economy" | "Premium" | "Business" | "First"
  >(props.flightClass?.cabinClass || "Economy");
  ...
  const calculateAllPassengerCount = () => {
    const all = adultCount + childrenCount + infantCount;
        ...
        switch={
          <div
            ...
            title={
              ...
              `${childrenCount + infantCount} Minor${
                childrenCount + infantCount === 1 ? "" : "s"
              }, ${cabinClass}`
            }
          >
            <div className="label">Passenger - Class</div>
            <p className="mb-0 reduced-info">
              {allPassengerCount} Passenger{allPassengerCount === 1 ? "" : "s"}
            </p>
        ...
        <div className="flight-class-case">
          <div className="sect-holder">
            ...
            <div className="passenger-sect">
              ...
              <IncrementalCountComponent
                updateCount={updateAdultCount}
                minimum={1}
                count={adultCount}
              />
            </div>
            <div className="passenger-sect">
              ...
              <IncrementalCountComponent
                updateCount={updateChildrenCount}
                count={childrenCount}
              />
            </div>
            <div className="passenger-sect">
          ...
          <div className="sect-holder pb-3">
            ...
            <div
              className={
                "flight-class-sect mb-2" +
                (cabinClass === "First" ? " selected" : "")
              }
            ...
            {adultCount >= infantCount ? (
              <button
                className="flight-button py-1 px-3"
                onClick={confirmPassengerCounts}
              >
                OK
              </button>
            ) : (
              <p className="reduced red-tx italic">
                You need an adult present for every infant, infants can not
                exceed adults
              </p>
            )}
          </div>
