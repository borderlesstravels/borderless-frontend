...
function HostProfilePage(props: any) {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'reviews' | 'properties' | 'bookings' | string>(params.tab || 'profile');
  ...
  const viewBookings = () => {
    navigate(`/${routeConstants.hostBookings}`);
  }
    ...
    <>
      <div className='host-holder'>
            ...
            <div className='profile-tab-card h-balance'>
              <div className='spread-info mb-3 pb-1'>
                ...
                <div className='tab-button-sect '>
                  <button onClick={viewBookings} className='px-4'>Bookings</button>
                </div>
              </div>
        ...
        <div className='host-tab-content'>
          {activeTab === 'profile' && <ProfileSect notifications={notifications} />}
