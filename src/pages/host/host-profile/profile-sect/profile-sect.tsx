...
function ProfileSect({notifications} : {notifications: iNotification[]}) {
  ...
  const toggleDrawControll = (index: number) => {
    ...
    newNotification[index].open ? newNotification[index].open = false : newNotification[index].open = true;
    setNotificationList(newNotification);
  }

  const toggleFreeze = (index: number) => {
    const newNotification = [...notificationList];
    newNotification[index].frozen ? newNotification[index].frozen = false : newNotification[index].frozen = true;
    setNotificationList(newNotification);
  }
            ...
            <div className='notifications-holder'>
              ...
              <div className='notification-list'>
                {notificationList.map((item, index) => (
                  <div className={'notification-card' + (item.frozen ? ' frozen' : '')} key={index}>
                    {
                      item.type === 'booking' &&
                    ...
                    <div className='drawer-holder'>
                      ...
                      <div className='draw-control' onClick={() => toggleDrawControll(index)}>
                        <FontAwesomeIcon icon={item.open ? 'chevron-up' : 'chevron-down'} />
                      </div>
                    </div>
                    <div className='action-holder'>
                      <div className='action'>
                        <button className='reject-button' onClick={() => rejectBooking(item, index)}>Reject</button>
                        <span></span>
                        <button className='confirm-button' onClick={() => confirmBooking(item, index)}>Confirm</button>
                        <span></span>
                        <button className='view-button wb-view' onClick={() => viewBookingDetail(item.booking_id)}>View</button>
                      </div>
                      <button className='view-button mb-view' onClick={() => viewBookingDetail(item.booking_id)}>View Property</button>
                    </div>
                    {
                      item.frozen &&
                ...
                ))}
                {
                  ...
                  <div className='py-4 text-center'>
                    <p className='mb-0'>There are no notifications as at present</p>
                  </div>
                }
              </div>
            </div>
