...
function BookingsSect(props: any) {
  ...
  const [allBookingRecords, setAllBookingRecords] = useState<iAllBookingRecord>();
  const [mode, setMode] = useState<'all' | 'pending' | 'declined' | 'confirmed' | 'completed' | string>('all');
  ...
  const getStayBookingRecords = () => {
    ...
    sendRequest(
      ...
      (res: any) => {
        ...
        setBookingRecords(Array.isArray(res?.data?.bookings) ? res.data.bookings.reverse() : []);
        setLoading(1);
      },
      (err: any) => {
        setLoading(1);
          ...
          <>
              ...
              <div className='col-lg-3 col-sm-6 pb-3'>
                <div className='point-card'>
                  <div className='spread-info'>
                    ...
                    <p className='percent-rep'>({Math.round(((allBookingRecords?.pendingBookings?.length || 0) / (allBookingRecords?.bookings?.length || 1) *100))}%)</p>
                  </div>
                  <h5>{formatNumber(allBookingRecords?.pendingBookings?.length)}</h5>
                </div>
              </div>
              <div className='col-lg-3 col-sm-6 pb-3'>
                <div className='point-card'>
                  <div className='spread-info'>
                    ...
                    <p className='percent-rep'>({Math.round(((allBookingRecords?.confirmedBookings?.length || 0) / (allBookingRecords?.bookings?.length || 1)) *100)}%)</p>
                  </div>
                  <h5>{formatNumber(allBookingRecords?.confirmedBookings?.length)}</h5>
                </div>
              </div>
              <div className='col-lg-3 col-sm-6 pb-3'>
                <div className='point-card'>
                  <div className='spread-info'>
                    ...
                    <p className='percent-rep'>({Math.round(((allBookingRecords?.completedBookings?.length || 0) / (allBookingRecords?.bookings?.length || 1)) *100)}%)</p>
                  </div>
                  <h5>{formatNumber(allBookingRecords?.completedBookings?.length)}</h5>
                </div>
              </div>
                  ...
                  <div className='outer-spread'>
                    <div className='inner-spread'>
                      <div className='image-holder'>
                        <div className='airline-image' style={{backgroundImage: `url(${ booking.apartment_img})`}}></div>
                      </div>
                      <div className='stay-time-case'>
                        ...
                        <div className='stay-time'>
                          <div className=''>
                            ...
                            <h5 className='mb-0 number-medium reduced'>{formatDate(booking.check_in_date)}</h5>
                          </div>
                          <FontAwesomeIcon icon={'minus'} className='increased px-3' />
                          <div className=''>
                            <p className='mb-0 reduced-x faint-tx'>Check-Out</p>
                            <h5 className='mb-0 number-medium reduced'>{formatDate(booking.check_out_date)}</h5>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className='space-right'>
                          <div className='description-grid-40 pb-3'>
                            ...
                            <div className='space-left'>
                              ...
                              <p className='mb-0 reduced number-medium'>{formatTime(booking.check_in_time, true)}</p>
                            </div>
                          </div>
                    ...
                    <div className='text-center py-3'>
                      <button className='purple-button purple-shadow' onClick={() => viewDetails(booking._id)}>View Details</button>
                    </div>
                  </div>
                  {
              ...
              
            {
              ...
              <div className='detail-card py-5'>
                <p className='mb-0 increased'>You have no {mode !== 'all' && mode} booking Records yet</p>
              </div>
            }
          </>
        }
