...
import {
  swalDanger,
  swalSuccess,
} from "../../../../../services/utils/swal-utils";
import {
  iBookedAppartmentInfo,
  iFullShortletInfo,
...
function StayBookingSingleRecord(props: any) {
  ...
  const goToPreview = () => {
    navigate(`/${Path.stayPreview}/${stayDetails?.apartment_id}`);
  };
  const returnToBookings = () => {
    navigate(`/${Path.myBookings}/stay`);
  };
  const goHome = () => {
    navigate(`/`);
  };
      ...
      .then((result: any) => {
        if (result.isConfirmed) {
          ...
          sendRequest(
            {
              url:
                "host-profile/decline-booking/" +
                stayDetails?.booking_reference,
              method: "PUT",
            },
            (res: any) => {
  ...
  const writeReview = () => {
    ...
    if (!reviewInfo || reviewInfo.length < 6) {
      toast.warning(
        "You can not submit an empty review, please write a review with a minimum of 6 characters"
      );
      return;
    }
    setSubmitting(true);
        ...
        <div className="stay-tickets">
          <div className="preview-sect">
            <div className="spread-info-web pt-3 pb-2">
              <h5>
                <span className="orange-tx">
                  {stayDetails?.apartment_country}{" "}
                </span>{" "}
                <span className="px-2 fainter-tx"> &gt; </span>
                <span className="orange-tx">
                  {" "}
                  {stayDetails?.apartment_state}{" "}
                </span>{" "}
                <span className="px-2 fainter-tx"> &gt; </span>
                <span className="increased-soft">
                  {" "}
                  {stayDetails?.apartment_name}
                </span>
              </h5>
              <div className="spread-info py-2">
                <h6 className="purple-tx clickable mb-0" onClick={goToPreview}>
                  {" "}
                  <FontAwesomeIcon icon={"chevron-left"} /> View Stay Details
                </h6>
                {stayDetails && (
                  <button className="review-button" onClick={openEditModal}>
                    Give Review
                  </button>
                )}
              </div>
                ...
                <div className="col-lg-6 spread-col py-3">
                  <div className="text">
                    <div className="spread-info-front">
                      ...
                      <h5 className="mb-0 f700">
                        Reservation Status: {stayDetails?.status}
                      </h5>
                    </div>
                    <p></p>
                    {userType === "host" ? (
                      <p>
                        Your appartment ({stayDetails?.apartment_name}), located
                        at {stayDetails?.apartment_address},{" "}
                        {stayDetails?.apartment_city},{" "}
                        {stayDetails?.apartment_state},{" "}
                        {stayDetails?.apartment_country}, has been requested for
                        booking from{" "}
                        {formatTime(stayDetails?.check_in_time || "", true)},
                        {formatDate(stayDetails?.check_in_date || "")} to{" "}
                        {formatDate(stayDetails?.check_out_date || "")}.
                        <br />
                        <br />
                        {processingOrder === 4 && (
                          <p>
                            You have successfully declined the guest
                            reservation, go to bookings list to see more booking
                            records.
                          </p>
                        )}
                        {processingOrder === 2 && (
                          <p>
                            You have successfully accepted the guest
                            reservation, go to bookings list to see more booking
                            records.
                          </p>
                        )}
                        {processingOrder !== 4 && processingOrder !== 2 && (
                          <p>
                            Please confirm the property is in proper condition
                            before accepting a guest.
                          </p>
                        )}
                      </p>
                    ) : (
                      <>
                        {stayDetails?.status === "pending" && (
                          <p>
                            Your booking for Ceasar Luxury Apartments is
                            successful. We would automatically debit you from
                            your chosen payment option once the host confirms
                            availability of the stay within 24 hours.
                            <br />
                            <br />
                            We would notify you on the progress of your booking
                            via email and in-app notification. Alternatively,
                      ...
                      {processingOrder === 2 || processingOrder === 4 ? (
                        ...
                        <div className="action">
                          <button
                            className={
                              "reject-button" +
                              (processingOrder === 1 || processingOrder === 3
                                ? " deactivated"
                                : "")
                            }
        ...
        <AppModal styleClass="" hideX onCloseModal={closeModal} ref={modalRef}>
          <div className="action-popup p-3 review-space">
            ...
            <div className="center-info pb-1 pt-4">
              <FontAwesomeIcon
                icon={"star"}
                className={"review-star" + (reviewStar >= 1 ? " active" : "")}
                onClick={() => setReviewStar(1)}
              />
              <FontAwesomeIcon
                icon={"star"}
                className={"review-star" + (reviewStar >= 2 ? " active" : "")}
                onClick={() => setReviewStar(2)}
              />
              <FontAwesomeIcon
                icon={"star"}
                className={"review-star" + (reviewStar >= 3 ? " active" : "")}
                onClick={() => setReviewStar(3)}
              />
              <FontAwesomeIcon
                icon={"star"}
                className={"review-star" + (reviewStar >= 4 ? " active" : "")}
                onClick={() => setReviewStar(4)}
              />
              <FontAwesomeIcon
                icon={"star"}
                className={"review-star" + (reviewStar >= 5 ? " active" : "")}
                onClick={() => setReviewStar(5)}
              />
            </div>
            {reviewStar ? (
              <p className="number-medium text-center">
                {reviewStar} Star ({reviewStar === 1 && "Poor Service"}
            ...
            <div className="info-grid w90 max500 py-3">
              <button
                ...
                className={"reject-button" + (submitting ? " deactivated" : "")}
              >
                {"Cancel"}
              </button>
              <button
                onClick={writeReview}
                className={"accept-button" + (submitting ? " deactivated" : "")}
              >
