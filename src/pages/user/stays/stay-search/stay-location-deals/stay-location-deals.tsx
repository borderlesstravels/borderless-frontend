...
import {
  stayLocationDealsData as dummyStayLocationDealsData,
  stayDummyImages,
} from "./stay-location-deals-data";
...
function StayLocationDealsSect(props: any) {
    ...
    sendRequest(
      ...
      (res: any) => {
        const stays = res.data.map((stay: any, index: number) => {
          return {
            location:
              stay?.outbound[stay?.outbound.length - 1]?.airport_from_details
                ?.city,
            country:
              stay?.outbound[stay?.outbound.length - 1]?.airport_from_details
                ?.country,
  ...
  const stayLocationDealsCarousel = stayLocationDealsData.map((data, index) => {
    return (
      <div className="location-deal-case" key={index}>
        <div
          className="deal-card"
          data-aos="zoom-in"
