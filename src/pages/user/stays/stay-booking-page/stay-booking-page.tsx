...
function StayBookingPage(props: any) {
  ...
  const bookShortlet = (values: FormikValues, controls: FormikHelpers<any>, price: number) => {
    console.log({values});
    // return
    sendRequest(
      {
            ...
            <div className='booking-card' data-aos="zoom-out">
              ...
              <Formik
                initialValues={{
                  ...
                  check_in_date: (params?.startDate ? new Date(params?.startDate) : new Date()).toISOString().split('T')[0],
                  check_out_date: (params?.endDate ? new Date(params?.endDate) : new Date(new Date().getTime() + (1000 * 60 * 60 * 24))).toISOString().split('T')[0],
                  check_in_time: "12:00:00", 
                  check_out_time: "12:00:00",
                }}
                validate={(values) => validate(values)}
                // onSubmit={(values, control) => console.log('bookShortlet(values, control)')}
                onSubmit={(values, control) => bookShortlet(values, control, calculateBookingSubtotal(stayDetails, values))}
              >
                {
