              ...
              {!loading && (
                <p className="reduced-x">
                  <span className="blue-tx number-bold">
                    Showing {filteredFlightList.length} of
                  </span>
                  <span className="orange-tx number-bold">
                    {" "}
                    {filteredFlightList.length} flights
                  </span>
                </p>
              )}
                  ...
                  <div className="flight-card" key={index}>
                    <div className="logo-side">
                      <img
                        ...
                        alt=""
                      />
                      <p className="text-center pt-2 mb-1">
                      ...
                      <div className="spread-info mb-2">
                        <div className="spread-info">
                          ...
                          <h6 className="mb-0 mx-3">
                            {getFlightToAndFrom(flight).from} -{" "}
                            {getFlightToAndFrom(flight).to}
                          </h6>
                        </div>
                        <h5 className="orange-tx number-bold">
                          <span className="reduced-im">{flight.currency}</span>{" "}
                          {formatNumber(Math.ceil(flight?.amount))}
                        </h5>
                      </div>
                      {flight.outbound.map((trip: any, tripIndex: number) => (
                        <div className="pb-2 w96 max500" key={tripIndex}>
                          <div className="spread-info pb-i">
                            <p className="f500 mb-0 number-bold">
                              {formatTime(trip.departure_time)}
                            </p>
                            <span className="reduced-im number-light">
                              {Math.floor(trip.duration / 60)}Hs,{" "}
                              {trip.duration % 60}Ms
                            </span>
                            <p className="f500 mb-0 number-bold">
                          ...
                          <div className="spread-info">
                            <p className="fainter-tx reduced-soft">
                              {trip.airport_from_details?.iata_code}
                            </p>
                            <span className="reduced-im faint-tx">
                              non-stop
                            </span>
                            <p className="fainter-tx reduced-soft">
                              {trip.airport_to_details?.iata_code}
                            </p>
                          </div>
                        </div>
                      ))}
