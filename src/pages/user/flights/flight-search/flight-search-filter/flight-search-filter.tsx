...
import {
  IFilter,
  initialFilter,
  validateAirlineFIlterInputs,
} from "./flight-filter-service";
...
function FlightSearchFilter({
  ...
  const updateSelectedTab = (
    ...
    switch (tab) {
      ...
      case "best":
        ...
        break;
      case "quickest":
        const quickest = [...acctiveList].sort(
  ...
  const validateFilter = (key: string, ev?: any) => {
    const { updatedFilter, changed } = validateAirlineFIlterInputs(
      key,
      filter,
      ev?.target?.value
    );
  ...
  const selectAirline = (airline: string) => {
    ...
    if (newFIlter.airlines.includes(airline)) {
      newFIlter.airlines = newFIlter.airlines.filter(
        (item) => item !== airline
      );
    } else {
  ...
  const searchFilter = () => {
    ...
    if (filter.airlines.length > 0) {
      ...
      filteredList.map((item: any) => {
        filter.airlines.map((airline) => {
          if (
            item?.outbound[0]?.airline_details?.name === airline &&
            !tempList.includes(item)
          ) {
            tempList.push(item);
          }
        });
    ...
    if (filter.earlyestTime) {
      ...
      filteredList.map((item: any) => {
        ...
        const flightTime = `2024-06-04T${
          item?.outbound[0]?.departure_time?.split("T")[1]
        }.000Z`;
        ...
        <div className="spread-info pb-4">
          ...
          {filterOpened && (
            <FontAwesomeIcon
              icon={"chevron-up"}
              className="increased"
              onClick={() => changeFilterState(false)}
            />
          )}
        </div>
        <div
          className={
            "filter " + (filterOpened ? "opened-filter" : "closed-filter")
          }
        >
          ...
          <div className="info-grid py-2">
            <div className="">
              ...
              <select
                ...
                onChange={(e) => {
                  updateFilter(e, "earlyestTime");
                  validateFilter("earlyestTime", e);
                }}
                className="simple-input"
              >
                <option value="">Choose time</option>
                {timeConstants.map((item, index) => (
            ...
            <div className="">
              ...
              <select
                ...
                onChange={(e) => {
                  updateFilter(e, "latestTime");
                  validateFilter("latestTime", e);
                }}
                className="simple-input"
              >
                <option value="">Choose time</option>
          ...
          <div className="info-grid py-2">
            <button
              type="button"
              className={
                "red-button" + (canFilter || activeFilter ? "" : " deactivated")
              }
              onClick={clearFilter}
            >
              Clear
            ...
            <button
              type="button"
              className={"purple-button" + (canFilter ? "" : " deactivated")}
      ...
      <div className="col-lg-9">
        <div className="pad-web">
          <div className="category-tabs">
            <div
              className={
                "category-tab" + (selectedTab === "cheapest" ? " active" : "")
              }
              onClick={() => updateSelectedTab("cheapest")}
            >
            ...
            <div
              className={
                "category-tab" + (selectedTab === "best" ? " active" : "")
              }
              onClick={() => updateSelectedTab("best")}
            >
            ...
            <div
              className={
                "category-tab" + (selectedTab === "quickest" ? " active" : "")
              }
              onClick={() => updateSelectedTab("quickest")}
            >
              <h6 className="mb-0">Quickest</h6>
              <p className="mb-0 number-light">
                {Math.floor(quickestTime / 60)}Hs, {quickestTime % 60}Ms
              </p>
            </div>
          </div>
